...
local conditions = setmetatable({
	...
	UnitMaxHealth = function(_, unit) return unit and (not UnitIsDeadOrGhost(unit) or UnitIsFeignDeath(unit)) and UnitHealth(unit) == UnitHealthMax(unit) end,
	...
	UnitMaxMana = function(_, unit) return unit and not (not UnitIsDeadOrGhost(unit) or UnitIsFeignDeath(unit)) and UnitPower(unit) == UnitPowerMax(unit) end,
