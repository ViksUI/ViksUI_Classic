...
lib.createDebuffs = function(f)
	...
	b.PostCreateButton = T.PostCreateButton
	b.PostUpdateButton = T.PostUpdateButton
	f.Debuffs = b
end
...
lib.addEssence = function(self)
	if T.Mainline and T.class == "EVOKER" then
		-- Essence bar
...
lib.genHolyPower = function(self)
	if (T.Cata or T.Mists or T.Mainline) and C.unitframe_class_bar.holy == true and T.class == "PALADIN" then
		...
		for i = 1, maxHolyPower do
			...
			self.HolyPower[i]:SetSize((self:GetWidth()-(T.Classic and 2 or 6)) / maxHolyPower, 7)
			if i == 1 then
...
lib.genComboPoints = function(self)
	if C.unitframe_class_bar.combo == true and (T.class == "ROGUE" or T.class == "DRUID") then
		...
		for i = 1, maxComboPoints do
			...
			self.ComboPoints[i]:SetSize(213 / maxComboPoints, 7)
			if i == 1 then
