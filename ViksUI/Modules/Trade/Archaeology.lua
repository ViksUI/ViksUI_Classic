...
function stArch:OnLoad(self)
	...
	if ViksUISettingsPerChar.Archaeology == false then self:Hide() end
	-- Title Bar
	...
	stArch["close"]:SetScript("OnMouseUp", function() self:Hide() ViksUISettingsPerChar.Archaeology = false end)
...
SlashCmdList.STARCHAEOLOGIST = function()
	...
	if stArchFrame:IsShown() then ViksUISettingsPerChar.Archaeology = true else ViksUISettingsPerChar.Archaeology = false end
end
...
stArchFrame:HookScript("OnEvent", function(self)
	...
	if not select(3, GetProfessions()) then stArchFrame:Hide() ViksUISettingsPerChar.Archaeology = false end
	if ViksUISettingsPerChar.Archaeology == true and not self:IsShown() then self:Show() end
end)
...
b:SetScript("OnClick", function()
	if _G["stArchaeologyFrame"]:IsShown() then
		...
		ViksUISettingsPerChar.Archaeology = true
	end
...
f:SetScript("OnEvent", function(_, _, _, _, _, _, spellid)
	if select(2, UnitRace("player")) == "Dwarf" then return end
