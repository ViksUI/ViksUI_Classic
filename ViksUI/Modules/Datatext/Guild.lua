...
local displayString = join("", GUILD, ": %d")
local noGuildString = join("", "Find a Guild")
...
local function Update(self, event, ...)
	if IsInGuild() then
		...
		local _, _, totalOnline = GetNumGuildMembers()
		totalOnline = totalOnline or 0 -- FIX: always pass a number!
	...
	for i = 1, #guildTable do
		...
		if info[7] and info[1] ~= myname then
			...
			if UnitInParty(info[1]) or UnitInRaid(info[1]) then
				...
				grouped = ""
				menuList[2].menuList[menuCountInvites] = {text = format(levelNameString, levelc.r*255,levelc.g*255,levelc.b*255, info[3], classc.r*255,classc.g*255,classc.b*255, info[1], ""), arg1 = info[1], func = inviteClick}
			end
			menuCountWhispers = menuCountWhispers + 1
			menuList[3].menuList[menuCountWhispers] = {text = format(levelNameString, levelc.r*255,levelc.g*255,levelc.b*255, info[3], classc.r*255,classc.g*255,classc.b*255, info[1], grouped), arg1 = info[1], func = whisperClick}
		end
	end
