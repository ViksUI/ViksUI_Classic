...
if C.datatext.Guild and C.datatext.Guild > 0 then
    ...
    local function BuildGuildTable()
        ...
        for i = 1, GetNumGuildMembers() do
            ...
            if status == 1 then
                status = "|cffFFFFFF[|r|cffFF0000AFK|r|cffFFFFFF]|r"
            elseif status == 2 then
                status = "|cffFFFFFF[|r|cffFF0000DND|r|cffFFFFFF]|r"
            else 
                status = ''
            end
            ...
            if connected then 
                count = count + 1
                guildTable[count] = {name, rank, level, zone, note, officernote, connected, status, class}
            end
        end
            ...
            if info[7] and info[1] ~= myname then
                ...
                if UnitInParty(info[1]) or UnitInRaid(info[1]) then
                    ...
                    menuList[2].menuList[menuCountInvites] = {
                        text = format(levelNameString, levelc.r*255, levelc.g*255, levelc.b*255, info[3], classc.r*255, classc.g*255, classc.b*255, info[1], ""),
                        arg1 = info[1], notCheckable=true, func = inviteClick
                    }
                end
                menuCountWhispers = menuCountWhispers + 1
                menuList[3].menuList[menuCountWhispers] = {
                    text = format(levelNameString, levelc.r*255, levelc.g*255, levelc.b*255, info[3], classc.r*255, classc.g*255, classc.b*255, info[1], grouped),
                    arg1 = info[1], notCheckable=true, func = whisperClick
                }
            end
        end
